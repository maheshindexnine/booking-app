...
interface EventScheduleState {
  ...
  updateSchedule: (id: string, schedule: Partial<EventSchedule>) => Promise<void>;
...
export const useEventScheduleStore = create<EventScheduleState>((set, get) => ({
  ...
  getSchedules: async () => {
    try {
      ...
      const schedules = await eventScheduleService.getEventSchedules();
  ...
  createSchedule: async (schedule: Omit<EventSchedule, '_id'>) => {
    try {
      set({ isLoading: true, error: null });
      const newSchedule = await eventScheduleService.createEventSchedule(schedule);
      set(state => ({ 
  ...
  updateSchedule: async (id: string, schedule: Partial<EventSchedule>) => {
    try {
      set({ isLoading: true, error: null });
      const updatedSchedule = await eventScheduleService.updateEventSchedule(id, schedule);
      set(state => ({
